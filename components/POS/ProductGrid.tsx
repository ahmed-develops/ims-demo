    ...
    return products.filter(product => {
      const matchesCollection =
        ...
        (selectedFilter === 'Discounted'
          ...
          : product.collection === selectedFilter); // ✅ collection

      const matchesSearch =
        product.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        product.id.toLowerCase().includes(searchQuery.toLowerCase()); // ✅ _id
          ...
          <div className="relative flex-1 group">
            ...
            <form onSubmit={(e) => isScanMode ? handleBarcodeSubmit(e) : e.preventDefault()}>
              <input
                ...
                className={`w-full pl-11 pr-12 py-3.5 bg-gray-50 dark:bg-gray-800 border-2 rounded-2xl text-gray-900 dark:text-white outline-none transition-all shadow-sm font-bold text-sm ${isScanMode
        ...
        <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-hide">
          {filterCollections.map((cat) => (
            <button
              ...
              className={`px-5 py-2 rounded-xl whitespace-nowrap text-xs font-black uppercase tracking-widest transition-all ${selectedFilter === cat
          ...
          {filteredProducts.map((product) => {
            ...
            return (
              <div
                ...
                className={`bg-white dark:bg-gray-800 rounded-3xl p-4 shadow-sm border border-transparent flex flex-col h-full transition-all group ${isOutOfStock ? 'opacity-50 grayscale cursor-not-allowed' : 'cursor-pointer hover:shadow-xl hover:border-indigo-500/20'
                  }`}
              >
                ...
                {sizePickerProduct.sizes.map((s, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center justify-between p-5 rounded-3xl border-2 transition-all group ${isOut ? 'bg-gray-50 dark:bg-gray-900 border-transparent opacity-40 grayscale cursor-not-allowed' : 'bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 hover:border-indigo-600 hover:bg-indigo-50/50 dark:hover:bg-indigo-900/10 active:scale-95'
                        }`}
                    >
