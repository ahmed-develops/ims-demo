...
import { X, Gift, Barcode, ChevronRight, ChevronLeft, CheckCircle2, User, Package, AlertCircle, Trash2, Plus, Minus, Info } from 'lucide-react';
...
const PRWizard: React.FC<PRWizardProps> = ({ isOpen, onClose, products, onComplete }) => {
  ...
  const performScan = (query: string) => {
    ...
    if (found) {
      ...
      setError("SKU/Variant not recognized.");
      return false;
    }
  };
  ...
  const handleFinish = () => {
    if (!recipientName.trim()) {
      setError("Recipient identity is mandatory.");
      return;
    }
    if (selectedItems.length === 0) {
      setError("Add stock to release.");
      setStep(0);
      return;
    }
  ...
  const handleContinue = () => {
    if (step === 0) {
      if (searchQuery.trim()) {
        ...
        setError("Scan an item to continue.");
      }
    } else if (step === 2) {
      if (!recipientName.trim()) {
      ...
      <div className="bg-white dark:bg-gray-950 rounded-[1.5rem] shadow-2xl w-full max-w-md overflow-hidden flex flex-col border border-white/10 relative max-h-[85vh]">
        ...
        <div className="p-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50 flex justify-between items-center shrink-0">
            <div className="flex items-center gap-2.5">
                ...
                <div>
                    <h3 className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight">PR / Talent Release</h3>
                    <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest">Complimentary Dispatch</p>
                </div>
            </div>
            ...
            {steps.map((s, idx) => (
                <div key={idx} className="flex flex-col items-center gap-1 relative flex-1">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-black z-10 transition-all duration-500 ${
                        step >= idx ? 'bg-rose-500 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-400'
                    }`}>
                        <s.icon size={10} />
                    </div>
                    {idx < steps.length - 1 && (
                        <div className={`absolute left-1/2 top-3 w-full h-[1px] -z-0 ${step > idx ? 'bg-rose-500' : 'bg-gray-100 dark:bg-gray-800'}`}></div>
                    )}
                </div>
            ))}
        </div>
        ...
        <div className="p-6 flex-1 overflow-y-auto scrollbar-hide">
            {step === 0 && (
                <div className="text-center space-y-5 animate-in fade-in zoom-in-95 duration-300 py-2">
                    <div className="w-16 h-16 bg-gray-50 dark:bg-gray-900 rounded-full flex items-center justify-center mx-auto border border-gray-100 dark:border-gray-800">
                        <Barcode size={32} className="text-rose-500 animate-pulse" />
                    </div>
                    <div className="space-y-0.5">
                        <h4 className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight">Scan PR Stock</h4>
                        <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest">Authorize complimentary sample</p>
                    </div>
                    <form onSubmit={handleScanForm} className="max-w-[300px] mx-auto">
                        <input 
                            ...
                            className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border-2 border-rose-500/50 rounded-xl text-center font-black text-xs tracking-[0.1em] uppercase focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 outline-none transition-all dark:text-white"
                        />
            ...
            {step === 1 && (
                <div className="animate-in fade-in slide-in-from-right-4 duration-300 py-1 space-y-3">
                    <div className="flex justify-between items-center px-1">
                      <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest">PR Release Queue</h4>
                      <button onClick={() => setStep(0)} className="text-[10px] font-black text-rose-500 uppercase flex items-center gap-1">
                      ...
                      {selectedItems.map((item, idx) => (
                        <div key={idx} className="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-xl border border-gray-100 dark:border-gray-800 flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[8px] font-bold text-gray-400">Size {item.size.sizeInternal} — Free Issue</p>
                          </div>
                          <div className="flex items-center gap-2 bg-white dark:bg-gray-800 p-1 rounded-lg border border-gray-100 dark:border-gray-700">
                            ...
                            <button onClick={() => updateQty(idx, 1)} className="p-0.5 text-gray-400 hover:text-rose-500"><Plus size={12} /></button>
                          </div>
            ...
            {step === 2 && (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300 py-1">
                    <div className="space-y-1">
                        <label className="text-[8px] font-black text-gray-400 uppercase tracking-widest ml-1">Influencer / Recipient *</label>
                        <div className="relative">
                            ...
                            <input 
                                ...
                                className="w-full pl-9 pr-4 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-lg font-black text-[11px] uppercase outline-none focus:ring-4 focus:ring-rose-500/10 transition-all dark:text-white"
                                placeholder="Enter Identity"
                            />
                        </div>
                    </div>
                    <div className="p-3 bg-blue-50/50 dark:bg-blue-900/10 rounded-lg border border-blue-100/50 flex gap-3 text-blue-600 dark:text-blue-400">
                        <Info size={14} className="shrink-0" />
                        <p className="text-[8px] font-bold uppercase leading-tight">Amount will be automatically set to ₨ 0. This is a non-sale movement for audit tracking.</p>
                    </div>
                </div>
            )}
            ...
            {step === 3 && (
                <div className="text-center space-y-4 animate-in fade-in zoom-in-95 duration-500 py-1">
                    ...
                    <div>
                        <h4 className="text-lg font-black text-gray-900 dark:text-white uppercase tracking-tighter leading-none">Confirm PR Gift</h4>
                        <p className="text-[8px] text-gray-400 font-bold uppercase tracking-widest mt-1">Authorized zero-value release</p>
                    </div>
                    ...
                    <div className="bg-white dark:bg-gray-900 p-4 rounded-xl border border-dashed border-gray-200 dark:border-gray-800 text-left space-y-2 max-h-40 overflow-y-auto">
                        <div className="flex justify-between items-center pb-2 border-b border-gray-50 dark:border-gray-800">
                            <span className="text-[8px] font-black text-gray-400 uppercase">Talent</span>
                            <span className="text-[10px] font-black text-rose-500 uppercase">{recipientName}</span>
                        </div>
                        {selectedItems.map((item, idx) => (
                          <div key={idx} className="flex justify-between items-center text-[9px] py-1 border-b border-gray-50 dark:border-gray-800 last:border-0">
                            ...
                            <span className="font-black text-rose-500">x{item.quantity}</span>
                          </div>
                        ))}
                    </div>
                </div>
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-800 flex gap-2.5 bg-gray-50/30 dark:bg-gray-900/30 shrink-0">
            ...
            <button 
                ...
                className="flex-[2] py-2.5 bg-rose-500 text-white rounded-lg font-black text-[9px] uppercase tracking-[0.15em] shadow-lg shadow-rose-100 dark:shadow-none hover:bg-rose-600 transition-all flex items-center justify-center gap-1.5 group"
            >
                {step === 3 ? 'Authorize Free Issue' : 'Continue'} 
