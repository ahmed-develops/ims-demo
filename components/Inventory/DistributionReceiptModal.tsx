...
import { Truck, Printer, X, ClipboardCheck, User, Tag } from 'lucide-react';
...
const DistributionReceiptModal: React.FC<DistributionReceiptModalProps> = ({ transaction, onClose }) => {
  if (!transaction) return null;

  const handlePrint = () => {
    const printWindow = window.open('', '_blank', 'width=1000,height=800');
    if (!printWindow) {
      alert("Please allow popups to generate distribution vouchers.");
      return;
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[150] flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[95vh] transition-colors duration-200">
        ...
        <div className="p-5 text-center text-white bg-indigo-600">
            ...
            <h2 className="text-sm font-black uppercase tracking-[0.2em] leading-none">Stock Release Authorized</h2>
        ...
        <div id="gatepass-content" className="bg-white p-10 flex-1 overflow-y-auto">
            <div className="flex justify-between items-start mb-8 pb-6 border-b-2 border-gray-900">
                ...
                <div className="text-right">
                    <p className="text-[10px] font-black text-gray-900 uppercase">Release ID</p>
            ...
            <div className="grid grid-cols-2 gap-8 mb-10">
                <div className="space-y-4">
                    <div>
                        <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Movement Type</p>
                        <div className="flex items-center gap-2">
                             <span className="px-2 py-1 bg-gray-100 rounded text-[10px] font-black text-gray-800 uppercase">{transaction.type}</span>
                        </div>
                    </div>
                    <div>
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <div className="flex items-center gap-2 text-indigo-600">
                            <Tag size={12} />
                            <p className="text-[11px] font-black uppercase">{transaction.externalOrderId || 'N/A'}</p>
                        </div>
                    </div>
                    <div>
                        <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Recipient / Rider</p>
                        <div className="flex items-center gap-2 text-gray-800">
                            <User size={12} />
                            <p className="text-[11px] font-black uppercase">{transaction.recipientName || 'Unspecified'}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <table className="w-full text-left">
                    <thead>
                        <tr className="border-b-2 border-gray-100 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                            ...
                            <th className="py-3 text-center">Variant</th>
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {(transaction.items || []).map((item, idx) => (
                            <tr key={idx} className="text-[11px]">
                                <td className="py-4">
                                    <p className="font-black text-gray-900 uppercase truncate max-w-[200px]">{item.name}</p>
                                    <p className="text-[9px] font-mono text-gray-400">SKU: {item.id}</p>
                                </td>
                                <td className="py-4 text-center font-bold text-gray-500 uppercase">Size {item.selectedSize.sizeInternal}</td>
                                <td className="py-4 text-right font-black text-indigo-600 text-sm">{item.quantity}</td>
                            </tr>
                        ))}
                    </tbody>
                    <tfoot>
                        <tr className="border-t-2 border-gray-900">
                            <td colSpan={2} className="py-4 text-[10px] font-black uppercase tracking-widest text-gray-900">Gross Total Articles</td>
                            <td className="py-4 text-right font-black text-xl text-gray-900">{totalItems}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
            ...
            {/* Signature Area */}
            <div className="grid grid-cols-2 gap-10 mt-16 pt-10 border-t border-dashed border-gray-200">
                <div className="space-y-8 text-center">
                    <div className="h-px bg-gray-300 w-full"></div>
                    <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Issued By</p>
                    <p className="text-[10px] font-bold text-gray-900 uppercase -mt-6 italic">{transaction.cashierName}</p>
                </div>
                <div className="space-y-8 text-center">
                    <div className="h-px bg-gray-300 w-full"></div>
                    <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Received By</p>
                </div>
            </div>
            ...
            <div className="mt-16 text-center">
                ...
                />
                 <p className="text-[7px] font-black text-gray-300 uppercase tracking-[0.5em] mt-4">Security Verified Movement</p>
            </div>
        </div>
