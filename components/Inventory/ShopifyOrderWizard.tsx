...
import { X, ShoppingBag, Barcode, ChevronRight, ChevronLeft, CheckCircle2, Tag, User, Package, AlertCircle, Trash2, Plus, Minus, Search } from 'lucide-react';
import { Product, ProductSize } from '../../types';

interface ShopifyOrderWizardProps {
  isOpen: boolean;
  onClose: () => void;
  products: Product[];
  onComplete: (payload: {
    items: { product: Product; size: ProductSize; quantity: number }[];
    details: { orderId: string; recipient: string };
  }) => void;
}
...
const ShopifyOrderWizard: React.FC<ShopifyOrderWizardProps> = ({ isOpen, onClose, products, onComplete }) => {
  const [step, setStep] = useState(0);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedItems, setSelectedItems] = useState<WizardItem[]>([]);
  const [orderDetails, setOrderDetails] = useState({ orderNumber: '', customerName: '' });
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setSearchQuery('');
      setSelectedItems([]);
      setOrderDetails({ orderNumber: '', customerName: '' });
      setError(null);
    }
  }, [isOpen]);
  ...
  const handleFinish = () => {
    if (!orderDetails.orderNumber.trim()) {
      setError("Shopify Order Number is required.");
      return;
    }
    ...
    onComplete({
      items: selectedItems,
      details: { orderId: orderDetails.orderNumber, recipient: orderDetails.customerName }
    });
    onClose();
  };
  ...
  const handleContinue = () => {
    if (step === 0) {
      // If user typed something but didn't hit enter, try to scan it first
      if (searchQuery.trim()) {
        const success = performScan(searchQuery);
        if (success) {
          // Stay on step 0 to allow more scans, or move to step 1 if user clicks again?
          // For UX, if they click continue and scan is successful, we can move to review.
          setStep(1);
        }
      ...
      if (!orderDetails.orderNumber.trim()) {
        setError("Order ID is mandatory.");
        return;
      }
      setStep(step + 1);
    } else {
      ...
      <div className="bg-white dark:bg-gray-950 rounded-[1.5rem] shadow-2xl w-full max-w-md overflow-hidden flex flex-col border border-white/10 relative max-h-[85vh]">
        ...
        <div className="p-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50 flex justify-between items-center shrink-0">
            <div className="flex items-center gap-2.5">
                ...
                <div>
                    <h3 className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight">Shopify Dispatch</h3>
            ...
            {steps.map((s, idx) => (
                <div key={idx} className="flex flex-col items-center gap-1 relative flex-1">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-black z-10 transition-all duration-500 ${
                        step >= idx ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-400'
                    }`}>
                        <s.icon size={10} />
                    </div>
                    {idx < steps.length - 1 && (
                        <div className={`absolute left-1/2 top-3 w-full h-[1px] -z-0 ${step > idx ? 'bg-blue-600' : 'bg-gray-100 dark:bg-gray-800'}`}></div>
                    )}
                </div>
            ))}
        </div>
        ...
        <div className="p-6 flex-1 overflow-y-auto scrollbar-hide">
            {step === 0 && (
                <div className="text-center space-y-5 animate-in fade-in zoom-in-95 duration-300 py-2">
                    ...
                    <div className="space-y-0.5">
                        <h4 className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight">Add Items</h4>
                        <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest">Enter Article-Variant Code</p>
                    </div>
                    <form onSubmit={handleScanForm} className="max-w-[300px] mx-auto">
                        <input 
                            ...
                            placeholder="NM-W2-001-1.5"
                            className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border-2 border-blue-500/50 rounded-xl text-center font-black text-xs tracking-[0.1em] uppercase focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all dark:text-white"
                        />
                        <p className="mt-3 text-[7px] text-gray-400 font-bold uppercase tracking-[0.2em]">Enter to scan. {selectedItems.length} items added.</p>
                    </form>
                </div>
            )}
            ...
            {step === 1 && (
                <div className="animate-in fade-in slide-in-from-right-4 duration-300 py-1 space-y-3">
                    <div className="flex justify-between items-center px-1">
                      <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Review Logistics Queue</h4>
                      <button onClick={() => setStep(0)} className="text-[10px] font-black text-blue-600 uppercase flex items-center gap-1">
                      ...
                      {selectedItems.map((item, idx) => (
                        <div key={idx} className="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-xl border border-gray-100 dark:border-gray-800 flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[8px] font-bold text-gray-400">V-{item.size.sizeInternal} | SKU: {item.product.id}</p>
                          </div>
                          <div className="flex items-center gap-2 bg-white dark:bg-gray-800 p-1 rounded-lg border border-gray-100 dark:border-gray-700">
                            ...
                            <button onClick={() => updateQty(idx, 1)} className="p-0.5 text-gray-400 hover:text-blue-500"><Plus size={12} /></button>
                          </div>
            ...
            {step === 2 && (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300 py-1">
                    <div className="space-y-1">
                        <label className="text-[8px] font-black text-gray-400 uppercase tracking-widest ml-1">Shopify Order ID *</label>
                        <div className="relative">
                            <Tag className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-300" size={14} />
                            <input 
                                autoFocus
                                value={orderDetails.orderNumber}
                                onChange={e => { setOrderDetails({...orderDetails, orderNumber: e.target.value}); setError(null); }}
                                className="w-full pl-9 pr-4 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-lg font-black text-[11px] uppercase outline-none focus:ring-4 focus:ring-blue-500/10 transition-all dark:text-white"
                                placeholder="#10245"
                            />
                        </div>
                    </div>
                    <div className="space-y-1">
                        <label className="text-[8px] font-black text-gray-400 uppercase tracking-widest ml-1">Label / Courier</label>
                        <div className="relative">
            ...
            {step === 3 && (
                <div className="text-center space-y-4 animate-in fade-in zoom-in-95 duration-500 py-1">
                    ...
                    <div>
                        <h4 className="text-lg font-black text-gray-900 dark:text-white uppercase tracking-tighter leading-none">Confirm Dispatch</h4>
                        <p className="text-[8px] text-gray-400 font-bold uppercase tracking-widest mt-1">Approval for {selectedItems.reduce((a,b)=>a+b.quantity,0)} Items</p>
                    </div>
                    ...
                    <div className="bg-white dark:bg-gray-900 p-4 rounded-xl border border-dashed border-gray-200 dark:border-gray-800 text-left space-y-2 max-h-40 overflow-y-auto">
                        <div className="flex justify-between items-center pb-2 border-b border-gray-50 dark:border-gray-800">
                            <span className="text-[8px] font-black text-gray-400 uppercase">Shopify Ref</span>
                            <span className="text-[10px] font-black text-blue-600 uppercase">{orderDetails.orderNumber}</span>
                        </div>
                        {selectedItems.map((item, idx) => (
                          <div key={idx} className="flex justify-between items-center text-[9px] py-1 border-b border-gray-50 dark:border-gray-800 last:border-0">
                            ...
                            <span className="font-black text-indigo-500">x{item.quantity}</span>
                          </div>
                        ))}
                    </div>
                </div>
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-800 flex gap-2.5 bg-gray-50/30 dark:bg-gray-900/30 shrink-0">
            ...
            <button 
                ...
                className="flex-[2] py-2.5 bg-blue-600 text-white rounded-lg font-black text-[9px] uppercase tracking-[0.15em] shadow-lg shadow-blue-100 dark:shadow-none hover:bg-blue-700 transition-all flex items-center justify-center gap-1.5 group"
            >
                {step === 3 ? 'Process & Sync' : 'Continue'} 
...
export default ShopifyOrderWizard;