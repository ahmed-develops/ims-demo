...
import { X, Truck, Barcode, ChevronRight, ChevronLeft, CheckCircle2, Package, AlertCircle, Trash2, Plus, Minus } from 'lucide-react';
import { Product, ProductSize } from '../../types';

interface StoreTransferWizardProps {
  isOpen: boolean;
  onClose: () => void;
  products: Product[];
  onComplete: (payload: {
    items: { product: Product; size: ProductSize; quantity: number }[];
    details: { recipient: string };
  }) => void;
}
...
const StoreTransferWizard: React.FC<StoreTransferWizardProps> = ({ isOpen, onClose, products, onComplete }) => {
  ...
  const performScan = (query: string) => {
    ...
    if (found) {
      if (found.size.warehouseStock <= 0) {
        setError(`Insufficient Warehouse stock for ${found.product.id}-${found.size.sizeInternal}.`);
        return false;
      } else {
        setSelectedItems(prev => {
          ...
          if (existingIdx > -1) {
            ...
            if (newQty > found!.size.warehouseStock) {
              setError("Transfer limit reached for current WH stock.");
              return prev;
            }
      ...
      setError("Article Serial not found.");
      return false;
    }
  };
  ...
  const updateQty = (idx: number, delta: number) => {
    setSelectedItems(prev => {
      ...
      if (newQty > item.size.warehouseStock) {
        setError(`Max WH stock available is ${item.size.warehouseStock}`);
        return prev;
      }
  ...
  const handleFinish = () => {
    if (selectedItems.length === 0) {
      setError("Add items to transfer.");
      setStep(0);
      return;
    }
    ...
    onComplete({
      items: selectedItems,
      details: { recipient: 'Boutique Front-end' }
    });
    onClose();
  };
  ...
  const handleContinue = () => {
    if (step === 0) {
      if (searchQuery.trim()) {
        ...
        setError("Scan items for transfer.");
      }
    } else {
      setStep(step + 1);
    }
  ...
  const steps = [
    { label: 'Pick', icon: Barcode },
    { label: 'Audit', icon: Package },
      ...
      <div className="bg-white dark:bg-gray-950 rounded-[1.5rem] shadow-2xl w-full max-w-md overflow-hidden flex flex-col border border-white/10 relative max-h-[85vh]">
        ...
        <div className="p-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50 flex justify-between items-center shrink-0">
            <div className="flex items-center gap-2.5">
                ...
                <div>
                    <h3 className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight">Store Transfer</h3>
                    <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest">WH → Front-end</p>
                </div>
            </div>
        ...
        <div className="px-12 pt-5 flex justify-between items-center shrink-0">
            {steps.map((s, idx) => (
                <div key={idx} className="flex flex-col items-center gap-1 relative flex-1">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-black z-10 transition-all duration-500 ${
                        step >= idx ? 'bg-emerald-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-400'
                    }`}>
                        <s.icon size={10} />
                    </div>
                    {idx < steps.length - 1 && (
                        <div className={`absolute left-1/2 top-3 w-full h-[1px] -z-0 ${step > idx ? 'bg-emerald-600' : 'bg-gray-100 dark:bg-gray-800'}`}></div>
                    )}
                </div>
            ))}
        </div>
        ...
        <div className="p-6 flex-1 overflow-y-auto scrollbar-hide">
            {step === 0 && (
                <div className="text-center space-y-5 animate-in fade-in zoom-in-95 duration-300 py-2">
                    <div className="w-16 h-16 bg-gray-50 dark:bg-gray-900 rounded-full flex items-center justify-center mx-auto border border-gray-100 dark:border-gray-800">
                        <Barcode size={32} className="text-emerald-600 animate-pulse" />
                    </div>
                    ...
                    <form onSubmit={handleScanForm} className="max-w-[300px] mx-auto">
                        <input 
                            ...
                            placeholder="NM-W2-001-M"
                            className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border-2 border-emerald-500/50 rounded-xl text-center font-black text-xs tracking-[0.1em] uppercase focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all dark:text-white"
                        />
                        <p className="mt-3 text-[7px] text-gray-400 font-bold uppercase tracking-[0.2em]">Press Enter to add. Pricing is hidden.</p>
                    </form>
                </div>
            )}
            ...
            {step === 1 && (
                <div className="animate-in fade-in slide-in-from-right-4 duration-300 py-1 space-y-3">
                    <div className="flex justify-between items-center px-1">
                      <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Transfer Audit</h4>
                      <button onClick={() => setStep(0)} className="text-[10px] font-black text-emerald-600 uppercase flex items-center gap-1">
                      ...
                      {selectedItems.map((item, idx) => (
                        <div key={idx} className="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-xl border border-gray-100 dark:border-gray-800 flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[8px] font-bold text-gray-400">Variant {item.size.sizeInternal} | SKU: {item.product.id}</p>
                          </div>
                          <div className="flex items-center gap-2 bg-white dark:bg-gray-800 p-1 rounded-lg border border-gray-100 dark:border-gray-700">
                            ...
                            <button onClick={() => updateQty(idx, 1)} className="p-0.5 text-gray-400 hover:text-emerald-500"><Plus size={12} /></button>
                          </div>
            ...
            {step === 2 && (
                <div className="text-center space-y-4 animate-in fade-in zoom-in-95 duration-500 py-1">
                    ...
                    <div>
                        <h4 className="text-lg font-black text-gray-900 dark:text-white uppercase tracking-tighter leading-none">Confirm Transfer</h4>
                        <p className="text-[8px] text-gray-400 font-bold uppercase tracking-widest mt-1">Authorizing movement of {selectedItems.reduce((a,b)=>a+b.quantity,0)} units</p>
                    </div>
                    ...
                    <div className="bg-white dark:bg-gray-900 p-4 rounded-xl border border-dashed border-gray-200 dark:border-gray-800 text-left space-y-2 max-h-40 overflow-y-auto">
                        ...
                        {selectedItems.map((item, idx) => (
                          <div key={idx} className="flex justify-between items-center text-[9px] py-1 border-b border-gray-50 dark:border-gray-800 last:border-0">
                            <span className="font-bold dark:text-white truncate max-w-[180px]">{item.product.name} ({item.size.sizeInternal})</span>
                            <span className="font-black text-emerald-500">x{item.quantity}</span>
                          </div>
                        ))}
                    </div>
                    <div className="p-3 bg-amber-50 dark:bg-amber-900/10 rounded-lg border border-amber-100 dark:border-amber-800/50 text-[8px] font-bold text-amber-600 dark:text-amber-400 uppercase leading-tight">
                        Warning: This will increase store stock and decrease warehouse stock immediately upon authorization.
                    </div>
                </div>
            )}
        </div>
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-800 flex gap-2.5 bg-gray-50/30 dark:bg-gray-900/30 shrink-0">
            ...
            <button 
                onClick={step === 2 ? handleFinish : handleContinue}
                className="flex-[2] py-2.5 bg-emerald-600 text-white rounded-lg font-black text-[9px] uppercase tracking-[0.15em] shadow-lg shadow-emerald-100 dark:shadow-none hover:bg-emerald-700 transition-all flex items-center justify-center gap-1.5 group"
            >
                {step === 2 ? 'Authorize Transfer' : 'Continue'} 
                {step < 2 && <ChevronRight size={14} className="group-hover:translate-x-0.5 transition-transform" />}
            </button>
        </div>
      </div>
    </div>
...
export default StoreTransferWizard;
